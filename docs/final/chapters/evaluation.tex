...
\begin{figure}[!h]
\centering
\includegraphics[width=0.7\textwidth]{images/old-bias.png}
\caption{Best rule strings discovered under different bias values $\lambda$ (shown on horizontal axis)}
\label{fig:bias-tuning}
\end{figure}
Figure~\ref{fig:bias-tuning}, depicts the opposing nature of the two fitness metrics. As $\lambda$ approaches 1, the number of dead ends decreases and the solution path length increases. The opposite is true as $\lambda$ approaches 0. A bias of around $0.4$ appears to keep the value of both metrics relatively high. Figure~\ref{fig:bias-mazes} shows the top mazes produced for different bias values. Qualitatively these appear to optimize the metrics they are being tested against.\\
\begin{figure}[!h]
\centering
...
\todo{Redo bias tuning using most recent hyperparameters}
...
Increasing the population size does not appear to resolve matters as seen in Figure~\ref{fig:zebra-fail}.\\

\begin{figure}[!h]
\centering
            \subfloat[State paramaters]{\includegraphics[width=.5\textwidth]{images/zebra-params.png}}\hfill
            \subfloat[Control parameters]{\includegraphics[width=.5\textwidth]{images/zebra-derivs.png}}\hfill
            \caption{Evolution of \textit{zebrafish} pattern using evolutionary strategy (population size = 50)}
\label{fig:zebra-fail}
\end{figure}

\todo{Test changing laplacian}
