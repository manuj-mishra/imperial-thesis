...
In this chapter, we begin by outlining metrics used to evaluate individual CAs, and EAs as a whole. We then systematically analyse the life-like CA rulespace using these metrics. Finally, we evaluate the evolutionary algorithms developed in this project against each goal. We establish which types of rules are easier or harder to learn based on our systematic analysis. 
...
\begin{table} \label{tab:multires-loss}
    \centering\hfill
    \subfloat[Average convergence rate]{
        \begin{tabular}{|c||c|c|c|}
            \hline
...
\centering
            \hfill
            \subfloat[State paramaters]{\includegraphics[width=.4\textwidth]{images/flower-params.png}}\hfill
...
\centering
            \hfill
            \subfloat[State paramaters for \textit{zebrafish} pattern (pop size = 50, Laplacian = nine-point stencil)]{\includegraphics[width=.4\textwidth]{images/zebra-params.png}}\hfill
            \subfloat[State paramaters for \textit{soliton} pattern (population size = 25, Laplacian = Compeau operator)]{\includegraphics[width=.4\textwidth]{images/soliton-params.png}}\hfill
            \hfill
